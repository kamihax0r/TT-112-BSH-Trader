...
class TestInstruments(unittest.TestCase):
    ...
    @patch.object(SessionManager, 'get_session_token', return_value='mock_token')
    @patch('requests.get')
    def test_list_nested_futures_option_chains(self, mock_get, mock_session_token):
        self.set_mock(mock_get)
        result = self.instruments.list_nested_futures_option_chains('ES')
        self.assertEqual(result, {"data": "test"})
        mock_get.assert_called_once_with(
            'https://api.cert.tastyworks.com/futures-option-chains/ES/nested',
            headers={'Authorization': 'mock_token', 'Content-Type': 'application/json'}
        )

    @patch.object(SessionManager, 'get_session_token', return_value='mock_token')
    @patch('requests.get')
    def test_list_detailed_futures_option_chains(self, mock_get, mock_session_token):
        self.set_mock(mock_get)
        result = self.instruments.list_detailed_futures_option_chains('ES')
        self.assertEqual(result, {"data": "test"})
        mock_get.assert_called_once_with(
            'https://api.cert.tastyworks.com/futures-option-chains/ES',
            headers={'Authorization': 'mock_token', 'Content-Type': 'application/json'}
        )

    @patch.object(SessionManager, 'get_session_token', return_value='mock_token')
    @patch('requests.get')
    def test_list_future_options(self, mock_get, mock_session_token):
        self.set_mock(mock_get)
        result = self.instruments.list_future_options()
        self.assertEqual(result, {"data": "test"})
        mock_get.assert_called_once_with(
            'https://api.cert.tastyworks.com/instruments/future-options',
            headers={'Authorization': 'mock_token', 'Content-Type': 'application/json'},
            params={'symbol[]': None, 'option-root-symbol': None, 'expiration-date': None, 'option-type': None, 'strike-price': None}
        )

    @patch.object(SessionManager, 'get_session_token', return_value='mock_token')
    @patch('requests.get')
    def test_get_future_option(self, mock_get, mock_session_token):
        self.set_mock(mock_get)
        result = self.instruments.get_future_option('ESZ2')
        self.assertEqual(result, {"data": "test"})
        mock_get.assert_called_once_with(
            'https://api.cert.tastyworks.com/instruments/future-options/ESZ2',
            headers={'Authorization': 'mock_token', 'Content-Type': 'application/json'}
        )

    @patch.object(SessionManager, 'get_session_token', return_value='mock_token')
    @patch('requests.get')
    def test_list_future_option_products(self, mock_get, mock_session_token):
        self.set_mock(mock_get)
        result = self.instruments.list_future_option_products()
        self.assertEqual(result, {"data": "test"})
        mock_get.assert_called_once_with(
    ...
    @patch.object(SessionManager, 'get_session_token', return_value='mock_token')
    @patch('requests.get')
    def test_get_future_option_product(self, mock_get, mock_session_token):
        self.set_mock(mock_get)
        result = self.instruments.get_future_option_product('CME', 'LO')
        self.assertEqual(result, {"data": "test"})
        mock_get.assert_called_once_with(
            'https://api.cert.tastyworks.com/instruments/future-option-products/CME/LO',
            headers={'Authorization': 'mock_token', 'Content-Type': 'application/json'}
        )
